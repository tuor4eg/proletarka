        ...
        {e.topicTitles.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-0.5">
            {e.topicTitles.map((title) => (
              <span key={title} className="text-xs bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded">
                {title}
              </span>
            ))}
          </div>
...
export function PersonTimeline({ events }: { events: TimelineEvent[] }) {
  ...
  return (
    <div className="mb-8">
      <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">Хронология</h2>
      <div className="flex flex-col gap-0 border-l-2 border-gray-100 pl-4">
        {needsCollapse ? (
          <>
            {first.map((e) => <EventItem key={e.id} e={e} />)}

            {expanded ? (
              <>
                {middle.map((e) => <EventItem key={e.id} e={e} />)}
              ...
              <div className="relative py-2">
                ...
                <button
                  ...
                  className="text-xs text-gray-400 hover:text-gray-600 transition-colors ml-1"
                >
                  Ещё {middle.length} {plural(middle.length, "событие", "события", "событий")}
                </button>
              </div>
            )}
