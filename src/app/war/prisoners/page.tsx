...
import { formatWarPersonYears, getWarTopicPeople, WAR_PRISONERS_TOPIC_CODE } from "@/lib/warPeople"
...
export default async function WarPrisonersPage({ searchParams }: Props) {
    const { letter, page: pageParam } = await searchParams
    const page = Math.max(1, Number(pageParam) || 1)

    const { topic, people, availableLetters, fallbackMap } =
        await getWarTopicPeople(WAR_PRISONERS_TOPIC_CODE)
    ...
    return (
        <>
            <PageHero title="В фашистской неволе" />
