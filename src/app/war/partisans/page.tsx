...
import { formatWarPersonYears, getWarTopicPeople, WAR_PARTISANS_TOPIC_CODE } from "@/lib/warPeople"
...
export default async function WarPartisansPage({ searchParams }: Props) {
    const { letter, page: pageParam } = await searchParams
    const page = Math.max(1, Number(pageParam) || 1)

    const { topic, people, availableLetters, fallbackMap } =
        await getWarTopicPeople(WAR_PARTISANS_TOPIC_CODE)
