...
export async function getEventsByEntityId(entityId: number) {
  ...
  const map = new Map<number, { id: number; text: string; yearFrom: number | null; yearTo: number | null; yearsLabel: string | null; topicIds: number[] }>();
  for (const row of rows) {
    if (!map.has(row.id)) {
      map.set(row.id, { id: row.id, text: row.text, yearFrom: row.yearFrom, yearTo: row.yearTo, yearsLabel: row.yearsLabel, topicIds: [] });
    }
    if (row.topicId !== null) {
  ...
  await db.transaction(async (tx) => {
    for (const input of inputs) {
      ...
      const [event] = await tx
        .insert(events)
        .values({ code, entityId, text: input.text, yearFrom: input.yearFrom, yearTo: input.yearTo, yearsLabel: input.yearsLabel })
        .returning({ id: events.id });

      if (input.topicIds.length > 0) {
        await tx.insert(eventTopics).values(
          input.topicIds.map((topicId) => ({ eventId: event.id, topicId }))
        );
      }
    }
  });
